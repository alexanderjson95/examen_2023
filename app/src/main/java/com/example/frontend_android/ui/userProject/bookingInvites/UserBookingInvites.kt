...
import com.example.frontend_android.ui.dashboard.invites.bookingInvites.BookingInvitesDirections
import com.example.frontend_android.ui.dashboard.invites.bookingInvites.UserBookingsRequestStatusAdapter
import com.example.frontend_android.ui.dashboard.invites.bookingInvites.UserBookingsViewmodel
...
@AndroidEntryPoint
class UserBookingInvites : Fragment(R.layout.fragment_booking_invites)
    {
        private val vm: ProjectBookingsViewmodel by viewModels()
        private lateinit var adapter: ProjectBookingsRequestStatusAdapter
        ...
        override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
            ...
            adapter = ProjectBookingsRequestStatusAdapter(
                remove = { b -> vm.removeUserBooking(b) },
                accept = { b -> vm.changeUserBooking(b,BookingStatusType.INVITE) }
            )
            ...
            val btnProjectNav = view.findViewById<MaterialButton>(R.id.btnProjectNav)
            ...
            toggleFilter.setOnCheckedStateChangeListener { _, checkedIds ->
                ...
                val all = vm.projectbookings.value
                val filter = all.filter  { p ->
                    ...
                    /*
                        ...
                        1,0 = tillgänglig
                     */
                    val bList = listOf(p.status == BookingStatusType.AVAILABLE,p.status == BookingStatusType.AVAILABLE)
                    when(bList){
            ...
            toggleGroup.addOnButtonCheckedListener { _, checkedId, isChecked ->
                if (isChecked) {
                    when (checkedId) {
                        R.id.btnProjectNav  -> {
                            val action = UserBookingInvitesDirections.projectbookingInvToProjectInv(0L)
